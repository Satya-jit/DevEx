...
### Detected Endpoints:
{chr(10).join(f'- `{endpoint}`' for endpoint in endpoints) if endpoints else '- / (Home endpoint)\n- /products (Products endpoint)'}
...
### Core Technologies
{chr(10).join(f'- {tech}' for tech in tech_stack[:10]) if tech_stack else '- Flask 3.0.0\n- Requests 2.31.0'}
